...
#define ZE_DX_RET_FAILED(call) do { HRESULT __hr = (call); ZE_DX_CHECK_DEBUG_INFO(__hr); if (FAILED(__hr)) { ZE_BREAK(); return ZE::RHI::DX::Error::Make(__hr); } } while (false)
...
#define ZE_DX_RET_FAILED_EXPECT(call) do { HRESULT __hr = (call); ZE_DX_CHECK_DEBUG_INFO(__hr); if (FAILED(__hr)) { ZE_BREAK(); return std::unexpected(ZE::RHI::DX::Error::Make(__hr)); } } while (false)

// Check result of the call returning HRESULT and if failed (ignoring debug messages), return said error
#define ZE_DX_RET_FAILED_NO_DEBUG(call) do { HRESULT __hr = (call); if (FAILED(__hr)) { ZE_BREAK(); return ZE::RHI::DX::Error::Make(__hr); } } while (false)
...
#define ZE_DX_RET_FAILED_NO_DEBUG_EXPECT(call) do { HRESULT __hr = (call); if (FAILED(__hr)) { ZE_BREAK(); return std::unexpected(ZE::RHI::DX::Error::Make(__hr)); } } while (false)
