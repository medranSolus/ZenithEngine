...
#define	ZE_DX_EXCEPT(code) ZE::RHI::DX::DirectXException(__LINE__, __FILENAME__, code, ZE_DX_EXCEPT_MANAGER.GetMessages())
...
#define	ZE_DX_THROW_FAILED_INFO(call) ZE_DX_SET_DEBUG_WATCH(); (call); do { auto msg = ZE_DX_EXCEPT_MANAGER.GetMessages(); if (msg.size()) { ZE_BREAK(); throw ZE::RHI::DX::DirectXException(__LINE__, __FILENAME__, S_FALSE, std::move(msg)); } } while (false)
...
// Before using needs call to ZE_DX_ENABLE()
// To get accurate debug info first call ZE_DX_SET_DEBUG_WATCH() to catch only important messages
#define	ZE_DX_EXCEPT(code) ZE::RHI::DX::DirectXException(__LINE__, __FILENAME__, code)
