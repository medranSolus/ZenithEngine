...
namespace ZE::GFX
{
	...
	public:
		...
		constexpr void Barrier(Device& dev, Resource::GenericResourceBarrier* barriers, U32 count) const noexcept(!_ZE_DEBUG_GFX_API) { ZE_ASSERT(barriers && count, "Empty barriers!"); ZE_RHI_BACKEND_CALL(Barrier, dev, barriers, count); }
