...
#endif
		constexpr void Reset(Device& dev, Resource::PipelineStateCompute& pso) { ZE_API_BACKEND_CALL(backend, Reset, dev, pso); }
		constexpr void Reset(Device& dev, Resource::PipelineStateGfx& pso) { ZE_API_BACKEND_CALL(backend, Reset, dev, pso); }
