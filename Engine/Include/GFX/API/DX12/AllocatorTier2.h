...
namespace ZE::GFX::API::DX12
{
	...
	public:
		...
		ResourceInfo Alloc_4KB(Device& dev, U32 bytes, const D3D12_RESOURCE_DESC& desc) { return AllocAlignMinimalChunks<SMALL_CHUNK, HEAP_SIZE>(dev, memory, bytes, desc, HEAP_FLAG); }
		// Buffers, normal textures and small multisampled textures (smaller than 4MB)
		ResourceInfo Alloc_64KB(Device& dev, U32 bytes, const D3D12_RESOURCE_DESC& desc) { return AllocAlignBigChunks<NORMAL_CHUNK, HEAP_SIZE>(dev, memory, bytes, desc, HEAP_FLAG); }
		// Only multisampled textures
		ResourceInfo Alloc_4MB(Device& dev, U32 bytes, const D3D12_RESOURCE_DESC& desc) { return AllocAlignBigChunks<HUGE_CHUNK, HEAP_SIZE>(dev, memory, bytes, desc, HEAP_FLAG); }
		// Buffers that can be written fast into from CPU
		ResourceInfo AllocDynamicBuffer(Device& dev, U32 bytes, const D3D12_RESOURCE_DESC& desc) { return AllocAlignMinimalChunks<NORMAL_CHUNK, BUF_HEAP_SIZE>(dev, buffers, bytes, desc, BUF_HEAP_FLAG); }
