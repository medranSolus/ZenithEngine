...
PSOut main(float3 texPos : TEX_POSITION)
{
	...
	// Shadow test
	const float shadowLevel = GetShadowLevel(normalize(cb_worldData.CameraPos - position), lightDistance, directionToLight, splr_AW, shadowMap, cb_pbrData.ShadowMapSize);
	if (shadowLevel != 0.0f)
	{
