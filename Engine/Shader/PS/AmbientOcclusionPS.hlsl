...
Texture2D noiseMap : register(t24); // RG - random vector

float main(float2 tc : TEXCOORD) : SV_TARGET
{
	const float2 sphericNormal = tx_normal.Sample(splr_PR, tc).rg;
	if (sphericNormal.x == 0.0f && sphericNormal.y == 0.0f)
		return 1.0f;

	const float3 normal = DecodeNormal(sphericNormal);
	const float3 position = GetWorldPosition(tc, tx_depth.Sample(splr_PR, tc).x, cb_inverseViewProjection);
	...
	for (uint i = 0; i < size; ++i)
	{
		...
		const float sampleDepth = GetLinearDepth(tx_depth.Sample(splr_PR, float2(offset.x, 1.0f - offset.y)).x, cb_nearClip, cb_farClip) + cb_bias;
