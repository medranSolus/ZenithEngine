...
TEXTURE_EX(colorJittered, Texture2D<FfxFloat32x4>, 0, 4);
TEXTURE_EX(opaqueOnly, Texture2D<FfxFloat32x4>, 1, 5);
TEXTURE_EX(motionVectors, Texture2D<FfxFloat32x4>, 2, 6);
TEXTURE_EX(reactiveMask, Texture2D<FfxFloat32>, 3, 7);
TEXTURE_EX(transparencyCompositionMask, Texture2D<FfxFloat32>, 4, 8);
...
void StoreAutoReactive(const in FFX_MIN16_I2 pxCoord, const in FFX_MIN16_F2 reactive)
{
	ua_reactive[pxCoord] = reactive.x;
	ua_composition[pxCoord] = reactive.y;
}

void StorePrevPreAlpha(const in FFX_MIN16_I2 pxCoord, const in FFX_MIN16_F3 color)
{
	ua_prevColorPreAlpha[pxCoord] = color;
}

void StorePrevPostAlpha(const in FFX_MIN16_I2 pxCoord, const in FFX_MIN16_F3 color)
{
...
void main(const uint2 gid : SV_GroupID, const uint2 gtid : SV_GroupThreadID)
{
	FFX_MIN16_I2 dtid = FFX_MIN16_I2(gid * uint2(THREAD_WIDTH, THREAD_HEIGHT) + gtid);
	...
	outReactiveMask.y = ComputeTransparencyAndComposition(dtid, iPrevIdx);

    if (outReactiveMask.y > 0.5f)
    {
		outReactiveMask.x = ComputeReactive(dtid, iPrevIdx);
