...
namespace ZE::GFX::Pipeline::RenderPass::CACAO
{
	ExecuteData::~ExecuteData()
	{
		if (Initialized)
		{
			ZE_FFX_CHECK(ffxCacaoContextDestroy(&Ctx), "Error destroying CACAO context!");
		}
	}
	...
	Expected<UpdateOperation> Update(Device& dev, RendererPassBuildData& buildData, ExecuteData& passData) noexcept
	{
		if (passData.RenderSize != Settings::RenderSize)
		{
			...
			{
				ZE_FFX_LOG_RET_FAILED_EXPECT(ffxCacaoContextDestroy(&passData.Ctx), "Error destroying CACAO context!");
			...
			ZE_FFX_LOG_RET_FAILED_EXPECT(ffxCacaoContextCreate(&passData.Ctx, &cacaoDesc), "Error creating CACAO context!");
