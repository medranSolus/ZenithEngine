...
{
	IndexBuffer::IndexBuffer(GFX::Device& dev, const GFX::IndexData& data)
	{
		ZE_VK_ENABLE_ID();
		...
		uploadInfo.CreateInfo.size = Utils::SafeCast<VkDeviceSize>(data.Count) * data.IndexSize;
	...
	GFX::IndexData IndexBuffer::GetData(GFX::Device& dev, GFX::CommandList& cl) const
	{
		return {};
	}
}