...
namespace ZE::GFX::Pipeline::RenderPass::Base
{
	...
	public:
		...
		virtual GfxResPtr<GFX::Resource::IBindable> LinkBindable() { throw ZE_RGC_EXCEPT("Source \"" + GetName() + "\" cannot be used as bindable!"); }
		virtual GfxResPtr<Resource::IBufferResource> LinkBuffer() { throw ZE_RGC_EXCEPT("Source \"" + GetName() + "\" cannot be used as pipeline buffer resource!"); }
	};
}