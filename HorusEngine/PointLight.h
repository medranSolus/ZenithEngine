...
namespace GFX::Light
{
	...
	public:
		...
		inline void SetPos(const DirectX::XMFLOAT3& position) noexcept override { mesh.SetPos(position); lightCamera->SetPos(mesh.GetPos()); }
		...
		inline void Update(const DirectX::XMFLOAT3& delta, const DirectX::XMFLOAT3& deltaAngle) noexcept override { mesh.Update(delta, deltaAngle);  lightCamera->SetPos(mesh.GetPos()); }
		inline void UpdatePos(const DirectX::XMFLOAT3& delta) noexcept override { mesh.UpdatePos(delta);  lightCamera->SetPos(mesh.GetPos()); }
