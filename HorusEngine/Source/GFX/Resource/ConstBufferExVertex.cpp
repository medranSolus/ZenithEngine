#include "GFX/Resource/ConstBufferExVertex.h"
#include "GFX/Resource/GfxDebugName.h"
#include "GFX/Graphics.h"

namespace GFX::Resource
{
	ConstBufferExVertex::ConstBufferExVertex(Graphics& gfx, const std::string& tag,
	...
	std::string ConstBufferExVertex::GenerateRID(const std::string& tag,
		const Data::CBuffer::DCBLayoutElement& root, U32 slot,
		const Data::CBuffer::DynamicCBuffer* buffer) noexcept
	{
		return "E" + std::to_string(slot) + "V" + std::to_string(root.GetByteSize()) + "#" + tag;
	}

	GfxResPtr<ConstBufferExVertex> ConstBufferExVertex::Get(Graphics& gfx,
