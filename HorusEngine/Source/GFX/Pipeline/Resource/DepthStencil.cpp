...
namespace GFX::Pipeline::Resource
{
		...
		: IBufferResource(gfx, width, height), usage(usage)
	{
		...
		GFX_SET_RID(depthStencilView.Get());
	}
	...
	Surface DepthStencil::ToSurface(Graphics& gfx, bool linearScale) const
	{
		...
		{
			GFX_THROW_FAILED_INFO(GetContext(gfx)->CopySubresourceRegion(textureStaged.Get(), 0, 0, 0, 0,
		...
		for (U32 y = 0; y < GetHeight(); ++y)
		{
			...
			for (U32 x = 0; x < GetWidth(); ++x)
			{
				...
				default:
					throw IMG_EXCEPT("Cannot convert DepthStencil to Surface, wrong DXGI_FORMAT!");
				}
			}
		}
