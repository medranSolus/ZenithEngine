...
App::App(const std::string& commandLine) : window(1600, 900, windowTitle), renderer(window.Gfx())
{
	...
	cameras = std::make_unique<Camera::CameraPool>(std::make_unique<Camera::PersonCamera>(window.Gfx(), renderer, "Camera_1",
		1.047f, 0.01f, viewDistance, 90, 0, DirectX::XMFLOAT3(-4.0f, 0.0f, 0.0f)));
	...
	AddShape(std::make_shared<GFX::Shape::Model>(window.Gfx(), renderer, "Models/nanosuit/nanosuit.obj", DirectX::XMFLOAT3(0.0f, -8.0f, 6.0f), "Nanosuit_tex", 0.78f));
