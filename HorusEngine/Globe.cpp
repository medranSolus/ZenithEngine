...
namespace GFX::Shape
{
		...
		: BaseShape(gfx), Object(position, name), sizes(width, height, length)
	{
		...
		techniques.emplace_back(std::make_shared<Pipeline::Technique>("Outline Scale", false));
		techniques.back()->AddStep({ graph, "outlineGeneration", std::make_shared<Visual::OutlineWrite>(gfx, vertexLayout) });
		techniques.back()->AddStep({ graph, "outlineDraw", std::make_shared<Visual::OutlineMaskOffset>(gfx, name + "OutlineScale", std::move(Data::ColorFloat3(1.0f, 1.0f, 0.0f)), std::move(vertexLayout)) });
