...
namespace GFX::Shape
{
		...
		: Object(position, name), sizes(width, height, length)
	{
		...
		auto vertexShader = std::make_shared<Resource::VertexShader>(gfx, L"SolidVS.cso");
		auto bytecodeVS = vertexShader->GetBytecode();
		AddBind(vertexShader);
		AddBind(std::make_shared<Resource::PixelShader>(gfx, L"SolidPS.cso"));
		...
		AddBind(std::make_shared<Resource::ConstantPixelBuffer<Resource::ObjectConstantBuffer>>(gfx, buffer));
